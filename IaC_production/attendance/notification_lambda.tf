...
resource "aws_lambda_function" "Notification_teamd" {
  function_name = "Notification_teamd"
  ...
  vpc_config {
    subnet_ids         = [aws_subnet.Private_At_Notification_teamd.id]
    security_group_ids = [aws_security_group.Notification_sg_teamd.id]
  }
}
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:logs:ap-northeast-2:758733530144:log-group:/aws/lambda/Notification_teamd*:*"
        ]
      },
      {
        Effect = "Allow",
